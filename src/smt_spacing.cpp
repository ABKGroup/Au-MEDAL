...
void spacing_worker(SmtModel& m, std::vector<z3::expr>& out, const Config& cfg,
    ...
    for (const Arr& cp : curr_points) {
        ...
        // A BIDIRECTION layer runs on both axes, so it is neither HORIZONTAL nor
        // VERTICAL and this test used to fall through to the bare width. M1 is
        // BIDIRECTION here, and its via-enclosure pad is 290 wide (V1 190/2 + 50
        // enclosure, and extension 290), not the 160 the width branch reported.
        // The 130nm the model was missing is why a pad and an adjacent column's
        // M1 came out 35nm apart against a 180nm rule and only DRC caught it.
        // max_curr_ex already uses extension for the search radius, so the two
        // were inconsistent.
        else if (curr_dir == BIDIRECTION ||
                 (curr_dir == HORIZONTAL && is_horizontal) || (curr_dir == VERTICAL && !is_horizontal))
        ...
        for (size_t xi = x_start; xi < x_end; ++xi) {
            ...
            for (size_t yi = y_start; yi < y_end; ++yi) {
                ...
                else if (next_dir == BIDIRECTION ||
                         (next_dir == HORIZONTAL && is_horizontal) || (next_dir == VERTICAL && !is_horizontal))
        ...
        for (int norder = 0; norder < 2; ++norder) {
            ...
            for (size_t xi = x_start; xi < next_x_points.size(); ++xi) {
                ...
                auto scan_next_y = [&](long next_y) -> bool {
                    ...
                    // Clamp each slack at zero: a negative slack means the
                    // shapes can overlap on that axis, and squaring it made
                    // the prune claim a distance that does not exist,
                    // skipping every one-track diagonal M1 pad pair.
                    if (dist_sq(std::max(0.0, x_unit - max_curr_prune - max_next_prune),
                                std::max(0.0, y_unit - max_curr_prune - max_next_prune)) >= spacing_sq)
                        return false;
...
std::vector<z3::expr> SmtModel::add_adjacent_via_pad_spacing(const Config& cfg, const RoutingGraph& g) {
    std::vector<z3::expr> out;
    std::map<int, std::vector<Arr>> by_z;
    for (const Arr& p : g.nodes) by_z[(int)p[2]].push_back(p);

    for (auto& kv : by_z) {
        const int z = kv.first;
        const std::string& layer = cfg.routing_layers[z];
        const double spacing = spacing_rule(cfg, "S2S", layer, layer);
        if (spacing < 0) continue;
        const long low_enc = layer_enclosure(cfg, cfg.lower_via.at(layer), layer);
        const long up_enc = layer_enclosure(cfg, cfg.upper_via.at(layer), layer);
        const double half_w = cfg.width(layer) / 2.0;
        // Only a via whose enclosure sticks out past the wire draws a pad.
        // Where it does not (M2 has no Via1 enclosure rule), the pair is a
        // plain wire-vs-wire case that the linear family already owns, and
        // asserting it here without an exposure condition would forbid
        // legal routing.
        const bool pad_low = (double)low_enc > half_w;
        const bool pad_up = (double)up_enc > half_w;
        if (!pad_low && !pad_up) continue;
        const double reach_low = (double)low_enc;
        const double reach_up = (double)up_enc;

        std::set<long> xs, ys;
        for (const Arr& p : kv.second) { xs.insert(p[0]); ys.insert(p[1]); }
        const std::vector<long> xv(xs.begin(), xs.end()), yv(ys.begin(), ys.end());
        std::set<Arr> present(kv.second.begin(), kv.second.end());

        // HOR pads extend along x, VER pads along y, so a same-row x-neighbour
        // pair interacts through its HOR extents and a same-column y-neighbour
        // pair through its VER extents.
        struct Axis { const std::vector<long>* line; bool along_x; const char* venc; };
        const Axis axes[2] = {{&xv, true, "VIA_ENC_HOR"}, {&yv, false, "VIA_ENC_VER"}};
        for (const Axis& ax : axes) {
            for (size_t i = 0; i + 1 < ax.line->size(); ++i) {
                const long a_pos = (*ax.line)[i], b_pos = (*ax.line)[i + 1];
                const long step = b_pos - a_pos;
                const std::set<long>& cross = ax.along_x ? ys : xs;
                for (long c : cross) {
                    const Arr a = ax.along_x ? Arr{a_pos, c, z} : Arr{c, a_pos, z};
                    const Arr b = ax.along_x ? Arr{b_pos, c, z} : Arr{c, b_pos, z};
                    if (!present.count(a) || !present.count(b)) continue;
                    const bool linked = edge_in_graph(g, a, b);
                    z3::expr link = linked ? bool_var(edge_var_name(a, b)) : ctx.bool_val(false);
                    const std::string sfx_a[2] = {std::string(ax.venc) + "_L", std::string(ax.venc) + "_U"};
                    const double reach_a[2] = {reach_low, reach_up};
                    const bool has_pad[2] = {pad_low, pad_up};
                    // A node with no cross-layer edge to that level can never
                    // carry the enclosure, so the clause would be dead weight.
                    auto via_possible = [&](const Arr& n, int level) {
                        const int oz = level == 0 ? z - 1 : z + 1;
                        if (oz < 0 || oz >= (int)cfg.routing_layers.size()) return false;
                        return edge_in_graph(g, n, Arr{n[0], n[1], oz});
                    };
                    const bool a_ok[2] = {via_possible(a, 0), via_possible(a, 1)};
                    const bool b_ok[2] = {via_possible(b, 0), via_possible(b, 1)};
                    for (int ia = 0; ia < 2; ++ia) {
                        for (int ib = 0; ib < 2; ++ib) {
                            // At least one side must be a real pad, otherwise
                            // this is the wire-vs-wire case.
                            if (!has_pad[ia] && !has_pad[ib]) continue;
                            if (!a_ok[ia] || !b_ok[ib]) continue;
                            if (step - reach_a[ia] - reach_a[ib] >= spacing) continue;
                            z3::expr va = bool_var(point_var_name(sfx_a[ia], a[0], a[1], z));
                            z3::expr vb = bool_var(point_var_name(sfx_a[ib], b[0], b[1], z));
                            out.push_back(!(va && vb) || link);
                        }
                    }
                }
            }
