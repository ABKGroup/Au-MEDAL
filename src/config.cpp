...
void Config::compute_derived_geometry() {
    ...
    // Distance every horizontal track is pushed away from the nearer
    // rail. The derived value can leave track 1 inside the rail dead
    // band, so allow a config override.
    y_offset = option.value("y_offset", (power_width("M1") - width("M1")) / 2.0);
    x_unit = pitch.at("Gate") / 2.0;
    // Horizontal tracks stack the layer that runs along them, so the
    // spacing this has to clear is not always M1's. Allow an override.
    y_unit = (double)option.value("y_unit", (long)pitch.at("M1"));
}
