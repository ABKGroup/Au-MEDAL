...
struct Builder {
    ...
    void add_gate_points(const std::string& nn_n, const std::string& nn_p,
        ...
        // Whether the two rows must carry the same net is a process property:
        // ASAP7 requires it, IHP SG13G2 does not. Default to requiring it.
        if (cfg.opt_bool("require_matching_gate_nets", true))
            throw std::runtime_error("gate cannot be placed over active region (no gate matching)");
        // Give each row its own pin on its own side of the shared contact row,
        // which is left to neither so the two can never merge. graph.cpp prunes
        // whichever of these tracks sits over active for this column.
        const double mid_y = cfg.cell_height / 2.0;
        const long gate_row = (long)(mid_y + cfg.np_offset);
        std::vector<long> n_ys, p_ys;
        for (long y : gate_ys) {
            if (y < gate_row) n_ys.push_back(y);
            else if (y > gate_row) p_ys.push_back(y);
        }
        if (n_ys.empty() || p_ys.empty())
            throw std::runtime_error("split gate column has no track on one side");
        add_pin_to_net(nn_n, TERM_GATE, x, n_ys, layer, pn_flip, gnd_rail, power_rail);
        add_pin_to_net(nn_p, TERM_GATE, x, p_ys, layer, pn_flip, gnd_rail, power_rail);
    }
    ...
    void add_source_drain_points(const std::string& nn_n, const std::string& nn_p, long nfin_n,
        ...
        double y_offset = cfg.opt_bool("bulk_planar")
