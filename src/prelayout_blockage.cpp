...
void add_active_blockage(const AddFn& add, const LayerMapFn& lm, const OffsetFn& off, const Config& cfg,
    ...
    for (int row = 0; row < row_count; ++row) {
        ...
        for (size_t x = 2; x < pf.size(); x += 2) {
            ...
            if (maxy - miny) { add(lm("Active"), minx, miny, maxx, maxy, false); add(lm("Gate"), minx, miny, maxx, maxy, false); }
...
void PreLayout::build_blockage(const Config& cfg, const NetOrders& no,
    ...
    const bool bulk_planar = cfg.opt_bool("bulk_planar");
    OffsetFn off = [&](const char* a, const char* b) {
        if (bulk_planar && std::string(a) == "Gate" && std::string(b) == "Active")
            return static_cast<double>(cfg.opt_long("active_gate_extension"));
        return cfg.rules.at("offset").at(a).at(b).get<double>();
    };
    const long ch = cfg.cell_height;
    ...
    const double y_off = bulk_planar ? cfg.opt_long("active_y_offset")
